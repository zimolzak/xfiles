...
all_ratings = data.frame(ep = rep(x$ep, x$num), rating = rep(x$rating, x$num)) # unroll again for histogram purposes
...
scale_bar + geom_bar(stat="identity", width=1) + facet_wrap(~ep) + opts(strip.background = theme_blank(), strip.text.x = theme_blank())
